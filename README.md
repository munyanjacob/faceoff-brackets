...
3. **Configure environment variables** — copy `.env.example` to `.env.local` and fill in the values from step 2, plus:
...
(`vercel.json`, once daily on the Hobby plan). Locally, trigger it by hand
...
Use the same value you set for `CRON_SECRET` in `.env.local`.
